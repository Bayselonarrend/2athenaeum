...
Функция СоздатьПост(Знач Сообщество, Знач Текст, Знач МассивКартинок, Знач Рекламный = Ложь, Знач СсылкаПодЗаписью = "", Знач Параметры = "") Экспорт
    ...
    Для Каждого КартинкаПоста Из МассивКартинок Цикл
        ...
        СерверФото   = МетодыРаботыHttp.ЧислоВСтроку(Ответ["server"]);
		...
		Пока Не Сохранено Или Не Счетчик = 5 Цикл
			Попытка 
				...
				Сохранено			= Истина;
			Исключение
				JSONСтрока = МетодыРаботыHttp.JSONСтрокой(Ответ);
				ИнструментарийВызовСервера.ЗаписьВЖурналИсключений(JSONСтрока);
				Счетчик = Счетчик + 1;
			КонецПопытки;		
		КонецЦикла;
...
Функция СохранитьКартинкуВАльбом(Знач Сообщество, Знач IDАльбома, Знач Картинка, Знач Описание = "", Знач Параметры = "") Экспорт
...
Функция ПоставитьЛайк(Знач Сообщество, Знач IDПоста, Знач IDСтены = "", Знач Параметры = "") Экспорт
    ...
    _Параметры.Вставить("object"      , "wall" + МетодыРаботыHttp.ЧислоВСтроку(IDСтены) + "_" + МетодыРаботыHttp.ЧислоВСтроку(IDПоста));
...
Функция СделатьРепост(Знач Сообщество, Знач IDПоста, Знач IDСтены = "", Знач ЦелеваяСтена = "", Знач Рекламный = Ложь, Знач Параметры = "") Экспорт
    
    _Параметры  = ПолучитьСтандартныеПараметры(Параметры, Сообщество);

    Источник = ?(ЗначениеЗаполнено(IDСтены)		, МетодыРаботыHttp.ЧислоВСтроку(IDСтены), МетодыРаботыHttp.ЧислоВСтроку(_Параметры["owner_id"]));
    Приемник = ?(ЗначениеЗаполнено(ЦелеваяСтена), СтрЗаменить(МетодыРаботыHttp.ЧислоВСтроку(ЦелеваяСтена), "-", ""), МетодыРаботыHttp.ЧислоВСтроку(_Параметры["group_id"]));
...
Функция НаписатьСообщение(Знач Сообщество, Знач Текст, Знач IDПользователя, Знач communitytoken, Знач Клавиатура = "", Знач Параметры = "") Экспорт
...
Функция НаписатьКомментарий(Знач Сообщество, Знач IDПоста, Знач IDСтены = "", Знач Текст, Знач Параметры = "") Экспорт
...
Функция СоздатьРекламноеОбъявление(Знач Сообщество, Знач НомерКампании, Знач ДневнойЛимит, Знач НомерКатегории, Знач IDПоста, Знач IDКабинета, Знач Параметры = "") Экспорт
    ...
    СтруктураКампании.Вставить("link_url"                   , "https://vk.com/wall-" + _Параметры["group_id"] + "_" + МетодыРаботыHttp.ЧислоВСтроку(IDПоста));
...
Функция ПриостановитьРекламноеОбъявление(Знач Сообщество, Знач IDКабинета, Знач IDОбъявления, Знач Параметры = "") Экспорт
...
Функция ИзменитьЗаписьРекламногоОбъявления(Знач Сообщество, Знач IDПоста, Знач IDКабинета, Знач IDОбъявления, Знач Параметры = "") Экспорт
...
Функция ПолучитьСтандартныеПараметры(Знач Параметры = "", Знач Сообщество) Экспорт
	...
	owner = ?(СтрНачинаетсяС(РеквизитыСообщества.ВК_owner_id, "-"), РеквизитыСообщества.ВК_owner_id, "-" + РеквизитыСообщества.ВК_owner_id);
