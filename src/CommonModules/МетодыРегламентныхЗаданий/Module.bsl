...
Процедура РассылкаУведомлений() Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Напоминание = "Знали ли вы?"
			...
			+ " еще ничего не читали! Нужно наверстать упущенное!";
		
		Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Telegram) И Не ВыборкаДетальныеЗаписи.Остановлен Тогда
			
			МетодыРаботыTelegram.ОтправитьСообщение(ВыборкаДетальныеЗаписи.Telegram, Напоминание,, ВыборкаДетальныеЗаписи.Пользователь);
			Отправлено = Истина;
			
		ИначеЕсли ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.VK) Тогда
			
			Напоминание = СтрЗаменить(Напоминание, "*", """");
			МетодыБота.ОтправитьОтвет(ВыборкаДетальныеЗаписи.Пользователь, Напоминание);
			Отправлено = Истина;
