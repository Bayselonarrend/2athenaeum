...
Функция ОбработатьВходящееСообщение(Знач СтруктураПараметров) Экспорт
	...
	СекретВК 					= ИнструментарийВызовСервера.ЗначениеРеквизитаОбъекта(Справочники.Сообщества.Athenaeum, "ВК_communitysecret");
	...
	ИначеЕсли (ТекстСообщения = "Начать" ИЛИ ТекстСообщения = "Start") И ВидСоцСети = СоцСетьVK Тогда	
		...
		Если ЗначениеЗаполнено(Блок) Тогда
			...
			ОтправитьОтвет(ПользовательИБ, ТекстОтветаПроизведение, Истина);
			ОтправитьОтвет(ПользовательИБ, Блок, Истина);
	...
	Если (ТекстСообщения = "->" Или ТекстСообщения = "<-") Тогда
		...
		Иначе	
			...
			МассивСообщений.Добавить("Не может быть! Кажется, я забыл, что вы читали. Ужас! Пожалуйста, выберите нужную книгу еще раз на https://athenaeum.digital");
...
Процедура ОбработатьИзменениеОповещений(Знач ID, Знач СоцСеть, Знач Текст) Экспорт
	...
	МетодыРаботыTelegram.ОтправитьСообщение(ID, ТекстОтвета,, Пользователь);
...
Функция ОбработкаАвторизацииТелеграм(Знач ТекстСообщения, Знач ПользовательИБ, Знач ИмяПользователя, Знач ИдентификаторПользователя, Знач Никнейм, Знач НовыйПользователь)
	...
	Попытка
		...
		МетодыРаботыTelegram.ОтправитьСообщение(Назначение, Смайл,, ПользовательИБ);
		МетодыРаботыTelegram.ОтправитьСообщение(Назначение, ТекстОтвет,, ПользовательИБ);
		...
		Если ЗначениеЗаполнено(КодКниги)  Тогда
			...
			Если ЗначениеЗаполнено(ЗапрашиваемыйТекст) Тогда
				...
				МетодыРаботыTelegram.ОтправитьСообщение(Назначение, ТекстОтвета,, ПользовательИБ);
				МетодыРаботыTelegram.ОтправитьСообщение(Назначение, Блок, "Страницы", ПользовательИБ);
...
Функция ВернутьБлок(Знач ПоследнийТекст, Знач Направление, Знач ПользовательИБ, Знач ЭтоТГ = Ложь) Экспорт
	...
	Иначе
		Если ЭтоТГ Тогда МетодыРаботыTelegram.ОтправитьСообщение(ПользовательИБ.Telegram, "%F0%9F%98%AE", Ложь, ПользовательИБ); КонецЕсли;
	...
	Попытка 
		...
		Для Каждого Сообщение Из МассивСообщений Цикл
			...
			Если ВидСоцСети = Перечисления.СоцСети.VK Тогда
				...
				ОтправитьОтвет(ПользовательИБ, Сообщение, ОтправлятьКлавиатуру);
				
			ИначеЕсли 	ВидСоцСети = Перечисления.СоцСети.Telegram Тогда
				
				ОтправитьКлавиатуру = ?(ОтправлятьКлавиатуру, "Страницы", "Нет");
				МетодыРаботыTelegram.ОтправитьСообщение(ИдентификаторЧата, Сообщение, ОтправитьКлавиатуру, ПользовательИБ);	
	...
	Если ИнструментарийВызовСервера.ЗначениеРеквизитаОбъекта(ПользовательИБ, "РазрешилПисатьVK") Тогда
		...
		Если ЗначениеЗаполнено(КодКниги)  Тогда
			...
			Если ЗначениеЗаполнено(ЗапрашиваемыйТекст) Тогда
				...
				ОтправитьОтвет(ПользовательИБ, ТекстОтвета, Истина);
				ОтправитьОтвет(ПользовательИБ, Блок, Истина);
...
Функция ОтправитьОтвет(Знач Пользователь, Знач Текст, Знач Клавиатура = Ложь) Экспорт
	...
	МетодыРаботыVK.НаписатьСообщение(Атеней
			, Текст
			, СтрЗаменить(ВКПользователя, " ", "")
			, ИнструментарийВызовСервера.ЗначениеРеквизитаОбъекта(Атеней, "ВК_communitytoken")
			, МетодыРаботыVK.СформироватьКлавиатуру(Атеней, МассивДействий));
