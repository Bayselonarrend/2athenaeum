...
Функция takeDatagetData(Запрос)
	...
	Если Действие = "logout" Тогда
		...
		ПользовательВыхода = Сайт_Апи.ВернутьПользователяПоCookie(Печенька);
	...
	Иначе
		...
		СтруктураПользователя 	= Сайт_Апи.ВернутьДанныеПользователя(Печенька);
		ЗапросJSON 				= ИнструментарийВызовСервера.JSONСтрокой(СтруктураПользователя);
...
Функция getBooksListgetData(Запрос)
	...
	МассивКниг		= Сайт_Апи.ВернутьСписокЧтения(ПользовательИБ, Количество);
	
	JSONОтвет = ИнструментарийВызовСервера.JSONСтрокой(МассивКниг);
...
Функция getBookStatsendData(Запрос)
	...
	ПользовательИБ 	= Сайт_Апи.ВернутьПользователяПоCookie(Печенька);
	ДанныеПоКниге	= Сайт_Апи.ВернутьСписокЧтения(ПользовательИБ, , КнигаПоКоду);
	...
	КонецЕсли; 
	 ...
	 ОтветJson = ИнструментарийВызовСервера.JSONСтрокой(СтруктураОтвета);
