...
Процедура ПолучитьПользователяИТекст(Знач СтруктураПараметров
    ...
    Если ЗначениеЗаполнено(ИдентификаторПользователя) Тогда
        ...
        Если ВидСоцСети = СоцСетьVK Тогда
            ...
            ПользовательИБ    = ЗначенияПовтИсп.КэшируемыйПользовательВК(ИдентификаторПользователя);
            Смайлы            = Ложь;

        ИначеЕсли ВидСоцСети = СоцСетьTG Тогда
            
            ПользовательИБ    = ЗначенияПовтИсп.КэшируемыйПользовательТелеграм(ИдентификаторПользователя);
            Смайлы            = Истина;
