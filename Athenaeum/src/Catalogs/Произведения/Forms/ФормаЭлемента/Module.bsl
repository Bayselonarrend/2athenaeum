...
Процедура КартинкаОткрытие(Элемент, СтандартнаяОбработка)
	...
	Диалог.Заголовок 		= Строка("Выбирите файл");
...
Процедура СформироватьФайл(Команда)
	...
	Записать();
	СформироватьФайлНаСервере(Объект.Ссылка, Объект.Код, Объект.Наименование);
...
#Если НЕ ВебКлиент Тогда
  ...
  Если ВыбФайлы <> Неопределено Тогда
	  ...
	  Для Н = 0 По 999 Цикл
		ТекстСтрокой = СтрЗаменить(ТекстСтрокой, "&#" 	+ Строка(999 - Н) + ";", "");
		ТекстСтрокой = СтрЗаменить(ТекстСтрокой, Символы.ПС + Символы.ПС + "[" 	+ Строка(999 - Н) + "]" + Символы.ПС + Символы.ПС, "");
		ТекстСтрокой = СтрЗаменить(ТекстСтрокой, Символы.ПС + "[" 	+ Строка(999 - Н) + "]" + Символы.ПС, "");
		ТекстСтрокой = СтрЗаменить(ТекстСтрокой, "[" 	+ Строка(999 - Н) + "]", "");
	КонецЦикла;
...
Процедура СформироватьФайлНаСервере(Знач Ссылка, Знач Код, Знач Наименование)

	ТД = ФорматированиеТекстаСервер.СформироватьФайлКниги(Ссылка);
	ПутьКПапке = Справочники.Сайты.Athenaeum.ПапкаДанныхСайта 
					+ "\books\" 
					+ Код 
					+ "\files\"; 
	...
	ТД.Записать(ПутьКПапке + Наименование + ".pdf", ТипФайлаТабличногоДокумента.PDF );
...
Процедура ЗагрузитьНаСайтНаСервере(Книга)
	МетодыСайтаСервер.СформироватьКниги(ТекущаяДатаСеанса() + 60 * 60 * 24, Ложь, Книга);
	МетодыСайтаСервер.СформироватьДанные(Ложь);
