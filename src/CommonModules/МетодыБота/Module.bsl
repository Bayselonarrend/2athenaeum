...
Процедура ОбработатьЗаписьНовогоБлока(Знач СтруктураПараметров, Закончена)
    ...
    Если Не Закончена Тогда
		...
		МассивПараметровФонового.Добавить(ПользовательИБ);
		МассивПараметровФонового.Добавить(СловКПрогрессу);
		МассивПараметровФонового.Добавить(ЧтениеНовых);
		
		ИнструментарийВызовСервера.ВыполнитьФоном("МетодыБота.ОбработатьЗаписьДневногоПрогресса", МассивПараметровФонового);
...
Функция ОтправитьКнигуСПроверкойРазрешенияВК(Знач КодКниги, Знач ПользовательИБ)
    ...
    Если ИнструментарийВызовСервера.ЗначениеРеквизитаОбъекта(ПользовательИБ, "РазрешилПисатьVK") Тогда
        ...
        //РассылкаОтветов(ПеречислениеВК, ПользовательИБ, МассивСообщений, Ложь);
        Возврат 200;
