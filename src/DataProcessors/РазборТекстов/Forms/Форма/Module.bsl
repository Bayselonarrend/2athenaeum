...
Процедура ЗаписатьТекстНаСервере(ТекстФайла, СсылкаТекст)
	...
	Сообщить("Взять из формы произведений");
...
Процедура НайтиНаСервере()
	...
	//		НайтиСовпаденияВТексте(МассивТекст, ВыборкаДетальныеЗаписи.Ссылка, ВыборкаДетальныеЗаписи.Наименование);
	//	КонецЕсли;
	//	
	//КонецЦикла;
...
&НаСервере
Процедура НайтиСовпаденияВТексте(Знач МассивТекст, Знач ПроизведениеСсылка, Знач ПроизведениеНаименование)
		...
		Для Каждого ТПоиск Из Поиск Цикл
			Если СтрНайти(нРег(Слово), нРег(СокрЛП(ТПоиск.Значение))) <> 0 Тогда
				...
				Лог = Лог 
					+ Слово 
					+ " в тексте:   " 
					+ Строка(ПроизведениеСсылка) 
					+ " ("
					+ ПроизведениеНаименование 
					+ ") " 
					+ Символы.ПС 
					+ Символы.ПС 
					+ "-------------------------------------"
