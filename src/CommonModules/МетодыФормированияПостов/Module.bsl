...
Функция СформироватьПост(Знач Пост, Знач ЭтоРеклама = Ложь, Знач Сеть = "") Экспорт
    ...
    Попытка 
        ...
        ИначеЕсли ТипЗнч(Пост) = Тип("Структура") Тогда
			...
			Если Пост.Свойство("Вложение") Тогда
				//@skip-check wrong-string-literal-content
				Вложение = Пост["Вложение"];
			КонецЕсли;
		...
		СтруктураПараметров.Вставить("Вложение"			, Вложение);
...
Функция СформироватьОтрывок(Знач Произведение, Знач ТолькоШапка = Ложь) 
    ...
    Если Не ТолькоШапка Тогда
        ...
        Если ПроизведениеАвтор.НаСайте Тогда
            ...
            + Символы.ПС
            + Символы.ПС
            + "Если вы еще не знакомы с нашей библиотекой, "
            + "то подробнее можно узнать по кнопке «Библиотека» в меню сообщества"; 
        КонецЕсли;
...
Функция СтруктураДополнительныхПараметров(Знач СтруктураВходныхПараметров)
	...
	Если СтруктураВходныхПараметров.Свойство("Вложение") Тогда
		
		Вложение = СтруктураВходныхПараметров["Вложение"];
