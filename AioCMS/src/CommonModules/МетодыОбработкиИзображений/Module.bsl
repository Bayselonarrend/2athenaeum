...
Функция СформироватьБиографию(Знач Био) Экспорт
	...
	СтандартныйОтступСлева = 30;
	ОтсутпСверхуЗаголовок  = 65;
	ОтступСверхуПериод     = 120;
	
	ЗаголовокОсьX = Строка(СтандартныйОтступСлева + ОсьX);
	ЗаголовокОсьY = Строка(ОтсутпСверхуЗаголовок + ОсьY);
	ПериодОсьX    = Строка(СтандартныйОтступСлева + ОсьX);
	ПериодОсьY    = Строка(ОтступСверхуПериод + ОсьY + ДопОтступ);
	
	РасположениеМарки = ?(ЗначениеЗаполнено(ОбъектБио.РасположениеМарки)
		, ОбъектБио.РасположениеМарки
		, Перечисления.Углы.SouthWest);
		
	РасположениеМарки = Строка(РасположениеМарки);
