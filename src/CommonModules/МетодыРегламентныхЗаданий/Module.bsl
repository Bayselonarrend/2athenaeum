...
Процедура Запостить(Сообщество, УровеньПоста)
			...
			Если ВыборкаДетальныеЗаписи.Следующий() Тогда
				...
				ВК_Действия.Запостить(ТекущийПост,, ТекущийПост.Сообщество);
				ТГ_Действия.СоздатьПост(ТекущийПост,, ТекущийПост.Сообщество);
				//ВК_Действия.СоздатьРекламноеОбъявление(ТекущийПост);
				...
				Если Не  ТипЗнч(ТекущийПост.Содержание) = Тип("СправочникСсылка.Термины") 
					...
					Если Сообщество = Справочники.Сообщества.Athenaeum Тогда
						...
						Если Не Строка(КартинкаСТекстом) = "Нет" Тогда		
							ВК_Действия.СоздатьИсторию(КартинкаСТекстом, ТекущийПост.Сообщество);
							УдалитьФайлы(ИВФ);
						КонецЕсли;
						
					Иначе			
						ВК_Действия.СоздатьИсторию(ИВФ, ТекущийПост.Сообщество);		
					КонецЕсли;
...
Процедура ПостКнигДекады() Экспорт
	...
	ВК_Действия.Запостить(,СтруктураЗаписи, Справочники.Сообщества.Athenaeum); 
	...
	СтруктураЗаписи.Вставить("Текст", ТекстПоста);
	ТГ_Действия.СоздатьПост(,СтруктураЗаписи, Справочники.Сообщества.Athenaeum);
...
Процедура ПостДняРождения() Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ВК_Действия.Запостить(,СтруктураЗаписи, ВыборкаДетальныеЗаписи.ОсновноеСообщество); 
		ТГ_Действия.СоздатьПост(,СтруктураЗаписи, ВыборкаДетальныеЗаписи.ОсновноеСообщество);
...
Процедура ПостРекламыКниги() Экспорт
	...
	Данные 			= РаботаССистемойСервер.СформироватьРекламуКниги(СлучайнаяКнига);
	...
	ВК_Действия.Запостить(, СтруктураЗаписи, Справочники.Сообщества.Athenaeum, Ложь);
	ТГ_Действия.СоздатьПост(, СтруктураЗаписи, Справочники.Сообщества.Athenaeum);
...
Процедура ПостСложнопоста() Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ВК_Действия.ЗапоститьСложнопост(ВыборкаДетальныеЗаписи.Пост, ВыборкаДетальныеЗаписи.Сообщество);
		ТГ_Действия.ОтправитьСложнопост(ВыборкаДетальныеЗаписи.Пост, ВыборкаДетальныеЗаписи.Сообщество);
...
Процедура ПолучитьСтатистику() Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ВК_Действия.ПолучитьСтатистикуПостов(600, ВыборкаДетальныеЗаписи.Сообщество);
		
		Статистика = ВК_Действия.ПолучитьСтатистику(НачалоДня(ВчерашнийДень), КонецДня(ВчерашнийДень), ВыборкаДетальныеЗаписи.Сообщество).Получить("response");
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если Не ВыборкаДетальныеЗаписи.Удаление Тогда			
			Если СообществаПостинга.Получить(ВыборкаДетальныеЗаписи.Сообщество) = Неопределено Тогда
				...
				ПостОбъект.ВК_ID = ВК_Действия.Запостить(,СтруктураЗаписи, ВыборкаДетальныеЗаписи.Сообщество, ВыборкаДетальныеЗаписи.ЭтоРеклама);
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ВК_ID) Тогда
				ВК_Действия.УдалитьПост(ВыборкаДетальныеЗаписи.ВК_ID, ВыборкаДетальныеЗаписи.Сообщество); 
			КонецЕсли;
...
Процедура ПолучитьКурсыВалют() Экспорт
	...
	КурсыНаСегодня = КоннекторHTTP.GetJson("https://api.nbrb.by/exrates/rates?periodicity=0");
...
Процедура РепостБотов() Экспорт
	

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ИспользованиеБотовСрезПоследних.Бот КАК Бот,
		|	ИспользованиеБотовСрезПоследних.Группа КАК Группа,
		|	ИспользованиеБотовСрезПоследних.Период КАК Период
		|ПОМЕСТИТЬ ПоследниеРепосты
		|ИЗ
		|	РегистрСведений.ИспользованиеБотов.СрезПоследних КАК ИспользованиеБотовСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	ПоследниеРепосты.Бот КАК Бот,
		|	ПоследниеРепосты.Группа КАК Группа
		|ПОМЕСТИТЬ ПоследнийБот
		|ИЗ
		|	ПоследниеРепосты КАК ПоследниеРепосты
		|
		|УПОРЯДОЧИТЬ ПО
		|	ПоследниеРепосты.Период
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПоследнийБот.Бот КАК Бот,
		|	ПоследнийБот.Группа КАК Группа,
		|	ИспользованиеБотов.Пост КАК Пост
		|ПОМЕСТИТЬ РепостыБота
		|ИЗ
		|	ПоследнийБот КАК ПоследнийБот
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИспользованиеБотов КАК ИспользованиеБотов
		|		ПО ПоследнийБот.Бот = ИспользованиеБотов.Бот
		|			И ПоследнийБот.Группа = ИспользованиеБотов.Группа
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	ПорядокПостов.Пост КАК Пост
		|ПОМЕСТИТЬ НужныйПост
		|ИЗ
		|	РегистрСведений.ПорядокПостов КАК ПорядокПостов
		|ГДЕ
		|	ПорядокПостов.Опубликован = ИСТИНА
		|	И ПорядокПостов.Пост.ВК_НомерПоста <> 0
		|	И НЕ ПорядокПостов.Пост В
		|				(ВЫБРАТЬ
		|					РепостыБота.Пост
		|				ИЗ
		|					РепостыБота)
		|	И ПорядокПостов.Пост.Сообщество.РаскруткаБотами = ИСТИНА
		|
		|УПОРЯДОЧИТЬ ПО
		|	ПорядокПостов.ДатаПубликации УБЫВ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПоследнийБот.Бот КАК Бот,
		|	ПоследнийБот.Группа КАК Группа,
		|	НужныйПост.Пост КАК Пост
		|ИЗ
		|	ПоследнийБот КАК ПоследнийБот,
		|	НужныйПост КАК НужныйПост";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	
	ТаблицаПрокси = РаботаССистемойСервер.ПолучитьТаблицуПрокси();
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Для Каждого Прокси Из ТаблицаПрокси Цикл
			...
			Если(РаботаССистемойСервер.ПроверитьПрокси(Прокси.Адрес, Прокси.Порт)) Тогда
				Попытка
					ТекущийПрокси = Новый ИнтернетПрокси;
					ТекущийПрокси.Установить("https", Прокси.Адрес, Число(Прокси.Порт));
					ВК_Действия.СделатьРепост(ВыборкаДетальныеЗаписи.Пост, ВыборкаДетальныеЗаписи.Бот, ВыборкаДетальныеЗаписи.Группа, ТекущийПрокси);
					Адрес 	= Прокси.Адрес;
					Порт	= Прокси.Порт;
					ТаблицаПрокси.Удалить(Н);
					Прервать;
				Исключение
					ОбъектПрокси = Справочники.Прокси.НайтиПоНаименованию(Прокси.Адрес).ПолучитьОбъект();
					ОбъектПрокси.ВремяСмерти = ТекущаяДатаСеанса();
					ОбъектПрокси.Записать();
					ТаблицаПрокси.Удалить(Н);
		...
		МЗ = РегистрыСведений.ИспользованиеБотов.СоздатьМенеджерЗаписи();
		МЗ.Бот = ВыборкаДетальныеЗаписи.Бот;
		МЗ.Группа = ВыборкаДетальныеЗаписи.Группа;
		МЗ.Период = ТекущаяДатаСеанса();
		МЗ.Пост = ВыборкаДетальныеЗаписи.Пост;
		МЗ.Записать();
		
		МЗП = РегистрыСведений.ЛогPlexus.СоздатьМенеджерЗаписи();
		МЗП.Период				= ТекущаяДатаСеанса();
		МЗП.Бот 				= ВыборкаДетальныеЗаписи.Бот; 
		МЗП.Сообщество 			= ВыборкаДетальныеЗаписи.Группа;
		Если ТекущийПрокси <> "" Тогда
			МЗП.Прокси 				= Адрес + ":" + Порт;
		КонецЕсли;
		МЗП.Действие 			= "Репост своего";
		МЗП.ПабликМаскировки 	= ВыборкаДетальныеЗаписи.Пост.Сообщество;
		МЗП.Записать(Истина);
...
Процедура МаскировочныеПосты() Экспорт
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ИспользованиеПабликовМаскировки.Паблик КАК Паблик
		|ИЗ
		|	РегистрСведений.ИспользованиеПабликовМаскировки КАК ИспользованиеПабликовМаскировки
		|
		|УПОРЯДОЧИТЬ ПО
		|	ИспользованиеПабликовМаскировки.ПоследнееИспользование";
	
	РезультатЗапроса = Запрос.ВыполнитьПакетСПромежуточнымиДанными();
	
	Боты 	= РезультатЗапроса[0].Выгрузить();
	Паблики = РезультатЗапроса[1].Выгрузить();
	У		= 0;
	...
	Для Н = 0 По Боты.Количество()- 1 Цикл
		
		
		Если У > Паблики.Количество() - 1 Тогда
			У = 0;		
		КонецЕсли;
		
		Паблик = Паблики[У].Паблик;
		...
		Для Каждого Прокси Из ТаблицаПрокси Цикл
			...
			Если(РаботаССистемойСервер.ПроверитьПрокси(Прокси.Адрес, Прокси.Порт)) Тогда
				...
				ТекущийПрокси.Установить("https", Прокси.Адрес, Число(Прокси.Порт));
				Попытка
					ПоследнийПост = ВК_Действия.ПолучитьПоследнийПостСообщества("-" + Паблик.ВК_owner_id, Боты[Н].Бот, ТекущийПрокси);
					МЗП = РегистрыСведений.ЛогPlexus.СоздатьМенеджерЗаписи();
					МЗП.Период				= ТекущаяДатаСеанса();
					МЗП.Бот 				= Боты[Н].Бот; 
					МЗП.Сообщество 			= Боты[Н].Группа;
					МЗП.Прокси 				= Прокси.Адрес + ":" + Прокси.Порт;
					МЗП.Действие 			= "Получение поста";
					МЗП.ПабликМаскировки 	= Паблик;
					МЗП.Записать(Истина);
					ТаблицаПрокси.Удалить(Ш);
					Прервать;
				Исключение
					ОбъектПрокси = Справочники.Прокси.НайтиПоНаименованию(Прокси.Адрес).ПолучитьОбъект();
					ОбъектПрокси.ВремяСмерти = ТекущаяДатаСеанса();
		...
		СообществоСтруктура = Новый Структура;
		
		
		СообществоСтруктура.Вставить("ВК_owner_id", СтрЗаменить(Строка(ПоследнийПост["owner_id"]),"-", ""));
		
		Пост.Вставить("Сообщество"		, СообществоСтруктура);
		Пост.Вставить("ВК_НомерПоста"	, ПоследнийПост["id"]);
		...
		Для Каждого Прокси Из ТаблицаПрокси Цикл
			...
			Если(РаботаССистемойСервер.ПроверитьПрокси(Прокси.Адрес, Прокси.Порт)) Тогда
				...
				Попытка
					Ответ = ВК_Действия.СделатьРепост(Пост, Боты[Н].Бот, Боты[Н].Группа, ТекущийПрокси);
					МЗП = РегистрыСведений.ЛогPlexus.СоздатьМенеджерЗаписи();
					МЗП.Период				= ТекущаяДатаСеанса();
					МЗП.Бот 				= Боты[Н].Бот; 
					МЗП.Сообщество 			= Боты[Н].Группа;
					МЗП.Прокси 				= Прокси.Адрес + ":" + Прокси.Порт;
					МЗП.Действие 			= "Постинг";
					МЗП.ПабликМаскировки 	= Паблик;
					МЗП.Записать(Истина);
...
Процедура СоздатьОпросПоПисателям() Экспорт
	...
	ВК_Действия.СоздатьОпрос(ТекстВопроса
		, МассивОтветов
		, Справочники.Сообщества.Athenaeum
...
Процедура ПингСтраница() Экспорт
	
	ЗапросСтраниц	= Новый Запрос(МетодыСайтаСервер.ПолучитьСписокСтраниц());
	
	Страницы 		= ЗапросСтраниц.Выполнить().Выгрузить();
	Прокси 			= РаботаССистемойСервер.ПолучитьТаблицуПрокси();
	Агенты 			= РаботаССистемойСервер.ПолучитьТаблицуАгентов();

	ГСЧ = Новый ГенераторСлучайныхЧисел;
	
	СлучайнаяСтраница 	= Страницы[ГСЧ.СлучайноеЧисло(0, Страницы.Количество() - 1)];
	СлучайныйПрокси		= Прокси[ГСЧ.СлучайноеЧисло(0, Прокси.Количество() - 1)];
	СлучайныйАгент		= Агенты[ГСЧ.СлучайноеЧисло(0, Агенты.Количество() - 1)];
	
	ТекущийПрокси = Новый ИнтернетПрокси;
	ТекущийПрокси.Установить("https", СлучайныйПрокси.Адрес, Число(СлучайныйПрокси.Порт));
	
	Заголовки = Новый Соответствие;
	Заголовки.Вставить("User-Agent", СлучайныйАгент.Агент);
	
	СтруктураЗапроса = Новый Структура;
	СтруктураЗапроса.Вставить("Прокси"		, ТекущийПрокси);
	СтруктураЗапроса.Вставить("Заголовки"	, Заголовки);
	
	Попытка
		Ответ = КоннекторHTTP.Get(СлучайнаяСтраница.URL,, СтруктураЗапроса);
	Исключение
		ОбъектПрокси = СлучайныйПрокси.Ссылка.ПолучитьОбъект();
		ОбъектПрокси.ВремяСмерти = ТекущаяДатаСеанса();
		ОбъектПрокси.Записать();
		ПингСтраница();
	КонецПопытки;
	
	МЗ = РегистрыСведений.ЛогПингов.СоздатьМенеджерЗаписи();
	МЗ.Период 		= ТекущаяДатаСеанса();
	МЗ.Страница 	= СлучайнаяСтраница.URL;
	МЗ.Ответ 		= КоннекторHTTP.КакТекст(Ответ);
	МЗ.КодОтвета 	= Ответ.КодСостояния;
	МЗ.IP			= СлучайныйПрокси.Адрес;
	МЗ.Записать();
...
Процедура РассылкаУведомлений() Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Telegram) И Не ВыборкаДетальныеЗаписи.Остановлен Тогда
			...
			ТГ_Действия.ОтправитьСообщение(ВыборкаДетальныеЗаписи.Telegram, "Тест",, ВыборкаДетальныеЗаписи.Пользователь);
			Отправлено = Истина;
			
		ИначеЕсли ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Владелец()) Тогда
			
			ВК_Действия.ОтправитьОтвет(ВыборкаДетальныеЗаписи.Пользователь, "Тест");
			Отправлено = Истина;
		КонецЕсли;
