...
Процедура GET_ОбработатьНачало(Знач Данные) Экспорт
	...
	Попытка
		...
		Данные				 = ИнструментарийВызовСервера.ПараметрыЗапросаВСоответствие(Данные);
		Печенька             = Данные.Получить("cookie");
		Количество           = Данные.Получить("book");
		Назначение           = Данные.Получить("social");
		...
		Иначе
			...
			, МетодыРаботыHttp.JSONСтрокой(Данные));
...
Функция GET_ВКЛогин(Знач Данные) Экспорт
	...
	Данные		= ИнструментарийВызовСервера.ПараметрыЗапросаВСоответствие(Данные);
    Токен       = Данные.Получить("code");
    Печенька    = Данные.Получить("uuid");
    КодКниги    = "";
...
Функция GET_ДанныеПользователя(Знач Данные) Экспорт
	...
	Данные	  = ИнструментарийВызовСервера.ПараметрыЗапросаВСоответствие(Данные);
    Печенька  = Данные.Получить("cookie");    // Строка
    Действие  = Данные.Получить("action");    // Строка
    
    Если Печенька = Неопределено Тогда        
        Возврат Ответ.Вставить("code", 400);
    КонецЕсли;
...
Функция GET_СписокЧтения(Знач Данные) Экспорт
	...
	Попытка
		...
		Печенька         = Данные.Получить("cookie");
		Количество       = Данные.Получить("count");
		ПользовательИБ   = ВернутьПользователяПоCookie(Печенька);
...
Функция GET_ПрогрессПользователя(Знач Данные) Экспорт
	...
	Данные	         = ИнструментарийВызовСервера.ПараметрыЗапросаВСоответствие(Данные);
    Печенька         = Данные.Получить("cookie");
    НомерКниги       = Данные.Получить("book");    
...
Функция ШаблонСтруктурыПараметровТелеграм()
