...
Процедура ОбновитьКартинкуНаСервере()
	...
	Попытка
		...
		ИначеЕсли  ТипЗнч(Объект.Содержание) = Тип("СправочникСсылка.Цитаты") Тогда
			...
			ИзображениеСТекстом = МетодыОбработкиИзображений.СформироватьИзображениеСТекстом(Объект.Содержание
			...
			ИзображениеДД = Новый ДвоичныеДанные(ИзображениеСТекстом);
