...
Функция Get(Знач URL, Знач Параметры = "") Экспорт
    ...
    Соединение       = ЗначенияПовтИсп.КэшируемоеСоединение(СтруктураURL["Сервер"]);
...
// Параметры:
//  URL - Строка - URL
//  Параметры - Структура Из Строка - Поля данных для отправки, которые не файлы
//  Файлы - Массив из ДвоичныеДанные - Массив файлов для отправки
//  ТипКонтента - Строка - MIME (image/jpeg по умолчанию)
// 
// Возвращаемое значение:
//  Произвольный -  Чтение JSON на основе HTTP ответа
Функция Post(Знач URL, Знач Параметры, Знач Файлы, Знач ТипКонтента = "image/jpeg") Экспорт
    ...
    ЗаписьТекста        = Новый ЗаписьДанных(ТелоЗапроса, "UTF-8", ПорядокБайтов.LittleEndian, "", "", Ложь);
    ...
    Для Каждого Файл Из Файлы Цикл 
        ...
        Если ТипЗнч(Файл.Значение) = Тип("Строка") Тогда
            ЗаписьТекста.Записать(Новый ДвоичныеДанные(Файл.Значение));
        Иначе
        ...
        Исключение
            ИнструментарийВызовСервера.ЗаписьВЖурналИсключений("Не удаляется");
        КонецПопытки;
    ...
    Если Ответ.Заголовки.Получить("Content-Encoding") <> Неопределено Тогда
        Если Ответ.Заголовки.Получить("Content-Encoding") = "gzip" Тогда
            Возврат JsonВСтруктуру(РаспаковатьОтвет(Ответ));
        КонецЕсли;
    КонецЕсли;
    ...
    Исключение
        ИнструментарийВызовСервера.ЗаписьВЖурналИсключений(ОписаниеОшибки(), Ответ.ПолучитьТелоКакСтроку());
        ВызватьИсключение;
    КонецПопытки;
    
КонецФункции

// Формирование нового запроса по сохраненным данным запроса, пришедшего на http-сервис
// Используется для автотестов
// 
// Параметры:
//  Заголовки - Соответствие Из Строка - Заголовки
//  Параметры - Структура - Параметры URL
//  URL - Строка - URL
//  Тело - Строка -  Тело строкой, как правило JSON
// 
// Возвращаемое значение:
//  HTTPОтвет -  Запрос по параметрам сервиса
...
// 
// Возвращаемое значение:
//  Структура -  Отдельно адрес, отдельно сервер:
// * Сервер - Строка - Домен 
// * Адрес - Строка - Относительный URL, все, что после домена
...
// 
// Параметры:
//  ТекстДД - ДвоичныеДанные -  Текст ДД
//  Кодировка - Строка -  Кодировка
// 
// Возвращаемое значение:
//  Произвольный -  Json в структуру
Функция JsonВСтруктуру(Знач ТекстДД, Знач Кодировка = "utf-8") Экспорт
    
    ТекстСтрокой = ПолучитьСтрокуИзДвоичныхДанных(ТекстДД, Кодировка);
    РегистрыСведений.Логирование.ЗаписьВЛог(ТекстСтрокой, Истина);
    
    ЧтениеJSON = Новый ЧтениеJSON;
    ЧтениеJSON.ОткрытьПоток(ТекстДД.ОткрытьПотокДляЧтения());
...
// 
// Параметры:
//  Данные - Произвольный - Данные, которые можно сериализировать
// 
// Возвращаемое значение:
//  Строка -  JSON
Функция JSONСтрокой(Знач Данные) Экспорт
    
    РегистрыСведений.Логирование.ЗаписьВЛог(Данные, Ложь);
...
// Число в строку с удалением неразрывных пробелов
// 
// Параметры:
//  Число - Число - Число
// 
// Возвращаемое значение:
//  Строка -  Число в строку
Функция ЧислоВСтроку(Знач Число) Экспорт
    Возврат СтрЗаменить(Строка(Число), Символы.НПП, "");
КонецФункции

// Удаление или замена запрещенных символов
// 
// Параметры:
//  Текст - Строка - Текст
...
Функция ПараметрыЗапросаВСтроку(Знач Параметры) 
    ...
    РегистрыСведений.Логирование.ЗаписьВЛог(Параметры, Ложь);
