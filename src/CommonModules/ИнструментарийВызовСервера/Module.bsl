...
Функция UnicodeEscape(Знач ИсходнаяСтрока) Экспорт
	...
	Для Каждого ЭлементСтроки Из МассивСтроки Цикл
		...
		НомерСмивола   = ПереводВДесятичнуюСиситему(вРег(НачалоСтроки), 16);
		ТекущийЭлемент = Прав(ТекущийЭлемент, Символов - 4);
		ТекущийЭлемент = Символ(НомерСмивола) + ТекущийЭлемент;
...
Функция ПереводВДесятичнуюСиситему(Знач Значение, Знач Основание) Экспорт
