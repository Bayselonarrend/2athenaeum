...
Процедура ОбработатьВходящееСообщение(Знач СтруктураПараметров) Экспорт
    ...
    ЗапрашиваемыйТекст            = ЗначенияПовтИсп.КэшируемаяКнига(вРег(ТекстСообщения)); 
    Смайлы                        = Ложь;
    НовыйПользователь             = Ложь;
    СоцСетьVK                     = ПредопределенноеЗначение("Перечисление.СоцСети.VK");
    СоцСетьTG                     = ПредопределенноеЗначение("Перечисление.СоцСети.Telegram");
    ...
    ИначеЕсли Не НовыйПользователь Тогда
        ...
        МассивСообщений = ОтправитьОшибкуНеНайдено(Смайлы);
	...
	Если Не МассивСообщений.Количество() = 0 Или НовыйПользователь Тогда
		...
		МетодыБота.РассылкаОтветов(ВидСоцСети, ПользовательИБ, МассивСообщений, НовыйПользователь);
...
Функция АвторизоватьПользователяНаСайте(Знач Токен, Знач Печенька, Знач КодКниги = "") Экспорт
    ...
    URL = "https://api.athenaeum.digital/node/bot/vk_login?uuid=" 
        + Печенька 
...
Процедура ОбработатьЗаписьНовогоБлока(Знач СтруктураПараметров, Закончена)
    ...
    Если Не Закончена Тогда
		...
		ДП = ОбработатьЗаписьДневногоПрогресса(ПользовательИБ, СловКПрогрессу, ЧтениеНовых);
    ...
    Попытка
        ЗаписатьПрогресс(ДП, МЗ);        
        ЗафиксироватьТранзакцию();        
    Исключение
        ОтменитьТранзакцию();
        ЗаписатьПрогресс(ДП, МЗ);
        ИнструментарийВызовСервера.ЗаписьВЖурналИсключений(ОписаниеОшибки(), "Транзакция прогресса не прошла");
    КонецПопытки;
    
КонецПроцедуры

Функция ОбработатьЗаписьДневногоПрогресса(ПользовательИБ, СловКПрогрессу, ЧтениеНовых) Экспорт
    
    ТДН = ИнструментарийВызовСервера.ВернутьДеньНедели();
    ...
    Запрос.Текст =
        ...
        |   ДневнойПрогресс.КоличествоСлов Как КоличествоСлов,
        |   ДневнойПрогресс.Дата Как Дата
        |ИЗ
        |   РегистрСведений.ДневнойПрогресс КАК ДневнойПрогресс
        |ГДЕ
        |   ДневнойПрогресс.Пользователь = &Пользователь
        |   И ДневнойПрогресс.День = &День";
    
    Запрос.УстановитьПараметр("День", ТДН);
    ...
    Если ВыборкаДетальныеЗаписи.Следующий() Тогда
        ЗаполнитьЗначенияСвойств(ДП, ВыборкаДетальныеЗаписи);
    Иначе
...
Процедура ЗаписатьПрогресс(ДП, МЗ)

    МЗ.Записать(Истина);

    Если Не ТипЗнч(ДП) = Тип("Строка") Тогда
        ДП.Записать(Истина);
    КонецЕсли;

КонецПроцедуры

Процедура СоздатьСеанс(Знач Заполнение, Знач Печенька, Знач ПользовательИБ) Экспорт
...
Процедура ПолучитьОтветПоКниге(Знач КодКниги, Знач ПользовательИБ, МассивСообщений)
    ...
    ЗапрашиваемыйТекст  = ЗначенияПовтИсп.КэшируемаяКнига(вРег(КодКниги));
    РеквизитыТекста		= ИнструментарийВызовСервера.ЗначенияРеквизитовОбъекта(ЗапрашиваемыйТекст
    	, "Наименование,Автор");
    ...
    Если ЗначениеЗаполнено(ЗапрашиваемыйТекст) Тогда
        ...
        + "«" 
        + РеквизитыТекста.Наименование 
        + "», "
        + РеквизитыТекста.Автор 
        + "!";
...
Процедура РассылкаОтветов(Знач ВидСоцСети
	, Знач ПользовательИБ
	, Знач МассивСообщений
	, Знач НовыйПользователь) Экспорт
	...
	Попытка
        ...
        Если ВидСоцСети = ПредопределенноеЗначение("Перечисление.СоцСети.VK") Тогда
            
            Для Каждого Сообщение Из МассивСообщений Цикл
                ОтправитьОтвет(ПользовательИБ, Сообщение);
            КонецЦикла;

        ИначеЕсли ВидСоцСети = ПредопределенноеЗначение("Перечисление.СоцСети.Telegram") Тогда
...
Функция ОбработатьИзменениеОповещений(Знач ПользовательИБ, Знач ВидСоцСети, Знач Текст)
    ...
    Если ЗначениеЗаполнено(ПользовательИБ) Тогда
        ...
        Если Текст = "ежедневно" Тогда
            ПеречислениеПериод = ПредопределенноеЗначение("Перечисление.ПериодичностьУведомлений.Ежедневно");
        ИначеЕсли Текст = "еженедельно" Тогда
            ПеречислениеПериод = ПредопределенноеЗначение("Перечисление.ПериодичностьУведомлений.Еженедельно");
        Иначе 
            ПеречислениеПериод = ПредопределенноеЗначение("Перечисление.ПериодичностьУведомлений.Нет");
        КонецЕсли;
...
Функция ОтправитьНовуюСтраницу(Знач СтруктураПараметров, Знач ПоследнийТекст, Знач ПользовательИБ, Знач Смайлы)
    ...
    Иначе
        ...
        МассивСообщений.Добавить(
            ...
            + "Пожалуйста, выберите нужную книгу еще раз в каталоге https://athenaeum.digital");
    КонецЕсли;
...
Функция ОтправитьНовуюКнигу(Знач ЗапрашиваемыйТекст, Знач ПользовательИБ, Знач Смайлы)
    ...
    ТекстОтвета     = "Да, я знаю такую книгу!" 
    	+ " Это «" 
    	+ РеквизитыТекста.Наименование 
    	+ "», "
        + РеквизитыТекста.Автор 
...
Функция ОтправитьОшибкуНеНайдено(Знач Смайлы)
    ...
    МассивСообщений.Добавить(
                "Хммм... Нет, такой книги у меня нет. "
                + "Попробуйте ввести другой код из каталога https://athenaeum.digital");
...
Функция ВернутьБлок(Знач ПоследнийТекст
	...
	Если ЗначениеЗаполнено(ПоследнийТекст) Тогда
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТекстыПроизведенийСтраницы.Текст КАК Текст
		|ИЗ
		|	Справочник.ТекстыПроизведений.Страницы КАК ТекстыПроизведенийСтраницы
		|ГДЕ
		|	ТекстыПроизведенийСтраницы.Ссылка.Владелец = &Произведение
		|	И ТекстыПроизведенийСтраницы.Слов = &Слов";
...
Функция ОтправитьКнигуСПроверкойРазрешенияВК(Знач КодКниги, Знач ПользовательИБ) Экспорт
