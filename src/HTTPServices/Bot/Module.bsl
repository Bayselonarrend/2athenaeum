...
Функция tgbotlogIn(Запрос)	
	...
	Если ОтветОбъект.Свойство("message") Тогда
		...
		Никнейм						= ?(ОтветОбъект.message.from.Свойство("username")
			, ОтветОбъект.message.from.username
			, "");
	...
	ИначеЕсли  ОтветОбъект.Свойство("my_chat_member") Тогда
		...
		ПользовательИБ		= Справочники.Пользователи.НайтиПоРеквизиту("Telegram"
			, Строка(ОтветОбъект.my_chat_member.from.id)); 
	...
	ИначеЕсли ОтветОбъект.Свойство("callback_query") Тогда
		...
		Никнейм						= ?(ОтветОбъект.callback_query.from.Свойство("username")
			, ОтветОбъект.callback_query.from.username, "");
